...
select count(movie_id) as number_of_movies from ct_genre;
...
+---------------+-------------------+ */
-- Type your code below:
select genre, round(avg(duration),2) as avg_duration from genre as g
inner join movie as m
on g.movie_id = m.id
group by genre
...
select title, avg_rating, rank() over(order by avg_rating desc) as movie_rank from movie as m
...
+------------------+-------------------+---------------------+*/
-- Type your code below:
select production_company, count(id) as movie_count, rank() over(order by count(id) desc) as prod_company_rank from movie as m
inner join ratings as r
on m.id = r.movie_id
...
-- Type your code below:
select median_rating, count(movie_id) as movie_count from movie as m
inner join ratings as r
on m.id = r.movie_id
...
with top_genre as
(
	select genre, count(title) as movie_count, rank() over(order by count(title) desc) as genre_rank from movie as m
...
top_directors as
(
	select n.name as director_name, count(g.movie_id) as movie_count, rank() over(order by count(g.movie_id) desc) as director_rank from names as n 
...
+-------------------+-------------------+---------------------+*/
-- Type your code below:
select production_company, sum(total_votes) as vote_count, dense_rank() over(order by sum(total_votes) desc) as prod_comp_rank from movie as m
inner join ratings as r
on m.id = r.movie_id
group by production_company
limit 3;
...
select name as actor_name, total_votes, count(m.id) as movie_count, round(sum(avg_rating*total_votes)/sum(total_votes),2) as actor_avg_rating,
				rank() over(order by avg_rating desc) as actor_rank from movie as m 
inner join ratings as r
on m.id = r.movie_id 
inner join role_mapping as rm
on m.id=rm.movie_id 
...
select name as actress_name, total_votes, count(m.id) as movie_count, round(sum(avg_rating*total_votes)/sum(total_votes),2) as actress_avg_rating,
				rank() over(order by avg_rating desc) as actress_rank from movie as m
inner join ratings as r
on m.id = r.movie_id
inner join role_mapping as rm
on m.id=rm.movie_id
...
select genre, round(avg(duration),2) as avg_duration,
        ...
        avg(round(avg(duration),2)) over(order by genre rows 10 preceding) as moving_avg_duration from movie as m
inner join genre as g 
on m.id= g.movie_id
group by genre
order by genre;
...
+-------------------+-------------------+---------------------+*/
-- Type your code below:
select production_company, count(m.id) as movie_count, row_number() over(order by count(id) desc) as prod_comp_rank from movie as m
inner join ratings as r
on m.id=r.movie_id
...
select name, sum(total_votes) as total_votes, count(rm.movie_id) as movie_count, avg_rating,
        dense_rank() over(order by avg_rating desc) as actress_rank from names as n
inner join role_mapping as rm
on n.id = rm.name_id
inner join ratings as r
...
(
	select d.name_id, name, d.movie_id, m.date_published,
			lead(date_published, 1) over(partition by d.name_id order by date_published, d.movie_id) as next_movie_date from director_mapping d
	join names as n
    on d.name_id=n.id
	join movie as m
    on d.movie_id=m.id
...
avg_inter_days as
(
	select name_id, avg(diff) as avg_inter_movie_days from date_difference
	group by name_id
),
final_result as
(
	select d.name_id as director_id, name as director_name, count(d.movie_id) as number_of_movies,
		 round(avg_inter_movie_days) as inter_movie_days, round(avg(avg_rating),2) as avg_rating,
		 sum(total_votes) as total_votes,
         min(avg_rating) as min_rating, max(avg_rating) as max_rating,
		 sum(duration) as total_duration,
		 row_number() over(order by count(d.movie_id) desc) as director_row_rank from names as n
